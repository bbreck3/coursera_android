...
public class DownloaderTaskFragment extends Fragment {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		//Log.d("arrayList[0]", Integer.toString(arrayList.get(0)));
		Integer integerArr[] = new Integer[arrayList.size()];
		for (int i = 0; i < arrayList.size(); i++) {
			integerArr[i] = arrayList.get(i);
			//Log.d("IntegerArr", Integer.toString(integerArr[i]));
		}
		String data[] = downloaderTask.downloadTweets(integerArr);
			...
			mContext.sendOrderedBroadcast(new Intent(
					...
					new BroadcastReceiver() {
						...
						public void onReceive(Context context, Intent intent) {
							...
							Bundle bundle = intent.getExtras();//("MainActivity.IS_ALIVE")//getArguments();
							//int is_alive = bundle.getInt("MainActivity.IS_ALIVE");
							//int is_cancelled = bundle.getInt("RESULT_CANCELLED");
							//Log.d("DEBUG: ",Integer.toString(is_alive));
							//Log.d("DEBUG: ",Integer.toString(is_cancelled));
							String str = "Bundle: {";
							if(bundle!=null){
								for(String key: bundle.keySet()){
									str += " " + key + " =>" + bundle.get(key) + ";\n";
