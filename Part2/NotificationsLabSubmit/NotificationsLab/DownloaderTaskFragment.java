...
public class DownloaderTaskFragment extends Fragment {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		ArrayList<Integer> arrayList = bundle.getIntegerArrayList("friends");//(Integer)bundle.get("TAG_FRIEND_RES_IDS");
		String strArr[] = new String[arrayList.size()];
		Integer intArr[] = new Integer[arrayList.size()];
		// TODO: Start the DownloaderTasks

		for (int i = 0; i < arrayList.size(); i++) {
			strArr[i] = Integer.toString(arrayList.get(i));
			intArr[i]=arrayList.get(i);
			Log.d("IntegerArr", strArr[i]);
		}
		 mDownloaderTask.execute(intArr);
	}
	...
	public class DownloaderTask extends AsyncTask <Integer, Void, String[]>{
		...
		protected String[] doInBackground(Integer... resourceIDS) {
			Log.d("DEBUG -->", "ASYNCTASK: doInBackGround");
			return downloadTweets(resourceIDS);
		}
		// TODO: Uncomment this helper method
		// Simulates downloading Twitter data from the network
		private String[] downloadTweets(Integer resourceIDS[]) {
			Log.d("DEBUG -->", "ASYNCTASK: downloadTweets");
			...
			try {
				...
				Log.d("DEEUG ERROR--> ",e.getMessage());
			}
		...
		private void notify(final boolean success) {
			Log.d("DEBUG", "notify from DownloaderTaskFragmet");
			...
			mContext.sendOrderedBroadcast(new Intent(
					...
					new BroadcastReceiver() {
						...
						public void onReceive(Context context, Intent intent) {
							...
							Log.d("DEBUG", "Reached Onreceive");
							// TODO: Check whether or not the MainActivity
							// received the broadcast
							//getRe
							String action =intent.getAction();
							int result_code = getResultCode();
							Log.d("Result Code -->", Integer.toString(result_code));
							//if(getResultCode()!=Activity.RESULT_OK){
							if(getResultCode()!=MainActivity.IS_ALIVE){
							//if (Integer.toString(getResultCode()).equals(MainActivity.DATA_REFRESHED_ACTION)) {
							//if(true || false){
							Log.d("DEBUG: -->", "Made it passed true");
								...
								// successfully
								mContentView.setTextViewText(R.layout.custom_notification, "Download Successful");
								...
								Notification.Builder noti = new Notification.Builder(context)
										.setContentTitle("New Download Content")
										.setContentText("New Download data")
								...
								notificationManager.notify(MY_NOTIFICATION_ID, noti.build());
		...
		public void onPostExecute(String[] feeds){
			Log.d("DEBUG -->", "Rached OnPostExecute");
			if(mCallback!=null) {
